           ...
           //in this first loop I choose the jetA and count the trks of A 
            for(int kjet=0; kjet < genJetPt->size(); kjet++){
                ...
                for(int jjet=0; (jjet< genJetPt->size()); jjet++){
                    ...
                    if( ((*genJetPt)[jjet] < (*genJetPt)[ijet]*0.9)||((*genJetPt)[jjet] > (*genJetPt)[ijet]*1.1)  ) continue;      
...
std::cout<< "made 4" << endl;
                    ...
                    TFile* fS_tempA = new TFile(Form("pythia_batch_output/2_root_out_2/dijob_%s.root",subList.c_str()), "recreate");
                    for(int wtrk =1; wtrk <trackbin+1; wtrk++){
