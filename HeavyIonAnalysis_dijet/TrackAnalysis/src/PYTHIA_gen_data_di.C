...
// cout << "TEST 1" << endl;
    ...
    TH2D* hPairs_BB        = new TH2D("hPairs_BB","hPairs_BB",  trackbin, bin0,trackbin, ptbin, bin0, ptbin);
    ...
    //what is the def of genDau_pt, genJetPt
            ...
            for(int kjet=0; kjet < jetPt->size(); kjet++){
                ...
                for(int jjet=ijet+1; (jjet< jetPt->size()); jjet++){
                    ...
                    double n_G_ChargeMult_count2 = 0.0;
                    ...
                    for(int  A_trk=0; A_trk < NNtrk2; A_trk++ ){
                        
                        TVector3 dau_A0;
                        dau_A0.SetPtEtaPhi((double)(*dau_pt)[jjet][A_trk],(double)(*dau_eta)[jjet][A_trk],(double)(*dau_phi)[jjet][A_trk]);
                        TLorentzVector dau_A0_4(dau_A0,dau_A0.Mag());
                        
                        if((*dau_chg)[jjet][A_trk] == 0) continue;
                        if(fabs(dau_A0.Eta()) > 2.4)        continue;
                        if(fabs(dau_A0.Perp())  < 0.3)      continue;

                        //     daughter pt with respect to the jet axis                 pt With Respect To Jet 
                        double jet_dau_pt0    =  ptWRTJet(JetB, dau_A0);

                        if(jet_dau_pt0 >3.0) continue;

                        double jet_dau_eta0   = etaWRTJet(JetB, dau_A0);
                        if(jet_dau_eta0 > track_eta_lim) continue;
                        ...
                        double Atrk_weight = (hReco2D[thisEffTable]->GetBinContent(hReco2D[thisEffTable]->FindBin( (*dau_pt)[jjet][A_trk], (*dau_eta)[jjet][A_trk] )));
                        ...
                        for(int i = 0; i < trackbin; i++){
                            for(int j = 0; j < ptbin; j++){
                                if(tkBool[i] + T_ptBool[A_trk][j] == 2){
                                    int k_PU=0;
                                    if ((Ntrig[i][j])==0) continue;
                                    hEPDrawT[i][j][k_PU]->Fill(jet_dau_eta, jet_dau_phi, 1.0* jet_HLT_weight* jet_HLT_weight2/(Atrk_weight*Ntrig[i][j]));
                                }
                            }
                        }
                              ...
                              for(int wppt = 1; wppt < ptbin+1; wppt++){
                                  ...
                                  for(int wpPU = 1; wpPU < PUbin+1; wpPU++){
				                    ...
				                    //   float A_Jt[XENT]  = {0};
                                      ...
                                      for(long int i = 0; i < (XENT-1); i++){
                                          for(long int j = (i+1); j < XENT; j++){
                                            ...
                                            //   double WdeltaJt  = fabs(A_Jt[i]-A_Jt[j]);
                                              ...
                                              hBckrndShifted[wtrk-1][wppt-1][wpPU-1]->Fill(WdeltaEta, WdeltaPhi, 1);
                                              hBckrndShifted[wtrk-1][wppt-1][wpPU-1]->Fill(-WdeltaEta, WdeltaPhi, 1);
                                              hBckrndShifted[wtrk-1][wppt-1][wpPU-1]->Fill(WdeltaEta, -WdeltaPhi, 1);
                                              hBckrndShifted[wtrk-1][wppt-1][wpPU-1]->Fill(-WdeltaEta, -WdeltaPhi, 1);
                                              hBckrndShifted[wtrk-1][wppt-1][wpPU-1]->Fill(WdeltaEta, 2*TMath::Pi() - WdeltaPhi, 1);
                                              hBckrndShifted[wtrk-1][wppt-1][wpPU-1]->Fill(-WdeltaEta,2*TMath::Pi() - WdeltaPhi, 1);
                                      ...
                                      long int NENT_BB =  hPairs_BB->GetBinContent(wtrk, wppt);
                                      long int XENT_BB =  ((1+floor(sqrt(1+(4*2*backMult*NENT_BB))))/2) ;
                                      double T_ETA[XENT] = {0};
                                      double T_PHI[XENT] = {0};
				                    //   float A_Jt[XENT]  = {0};

                                      for(int x = 0; x<XENT_BB; x++){
                                          gRandom->SetSeed(0);
                                          double WEtaT, WPhiT;
                                      ...
                                      for(long int i = 0; i < (XENT-1); i++){
                                          for(long int j = (i+1); j < XENT; j++){
                                              ...
                                              double WdeltaEta_BB = (T_ETA[i]-T_ETA[j]);
                                              double WdeltaPhi_BB = (TMath::ACos(TMath::Cos(T_PHI[i]-T_PHI[j])));
                                            //   double WdeltaJt  = fabs(A_Jt[i]-A_Jt[j]);

                                              hBckrndShifted[wtrk-1][wppt-1][wpPU-1]->Fill(WdeltaEta_BB, WdeltaPhi_BB, 1);
                                              hBckrndShifted[wtrk-1][wppt-1][wpPU-1]->Fill(-WdeltaEta_BB, WdeltaPhi_BB, 1);
                                              hBckrndShifted[wtrk-1][wppt-1][wpPU-1]->Fill(WdeltaEta_BB, -WdeltaPhi_BB, 1);
                                              hBckrndShifted[wtrk-1][wppt-1][wpPU-1]->Fill(-WdeltaEta_BB, -WdeltaPhi_BB, 1);
                                              hBckrndShifted[wtrk-1][wppt-1][wpPU-1]->Fill(WdeltaEta_BB, 2*TMath::Pi() - WdeltaPhi_BB, 1);
                                              hBckrndShifted[wtrk-1][wppt-1][wpPU-1]->Fill(-WdeltaEta_BB,2*TMath::Pi() - WdeltaPhi_BB, 1);
                                            //   hMomBckrndShifted[wtrk-1][wppt-1][wpPU-1]->Fill(WdeltaJt, 1);
