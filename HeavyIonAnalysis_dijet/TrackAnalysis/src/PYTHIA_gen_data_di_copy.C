            ...
            for(int kjet=0; kjet < jetPt->size(); kjet++){
                ...
                //if((*chargedMultiplicity)[indicesR[kjet]] >= trackbinbounds[i] && (*chargedMultiplicity)[indicesR[kjet]] < trackbinboundsUpper[i]){
                    if(n_G_ChargeMult_count >= trackbinbounds[i] && n_G_ChargeMult_count < trackbinboundsUpper[i]){
                        tkBool[i] = 1;
                        // hJet_Pass           ->Fill(i);
                        hBinDist_gen[i]         ->Fill(n_G_ChargeMult_count, jet_HLT_weight);
                    }
                }
...
// Define a vector to store the selected particles
std::vector<std::complex<double>> selected_particles;

// Loop over all particles in the set
for (int i = 0; i < 5; i++) {
    // Extract the eta value of the particle
    double eta = get_eta(i);

    // If the eta value is less than 4, add the particle to the selected_particles vector
    if (eta < 4) {
        selected_particles.push_back(std::polar(1.0, get_phi(i)));
    }

    // If the eta value is greater than 4 and the condition eta3 - eta1 > 2 is satisfied, add the particle to the selected_particles vector
    if (eta > 4 && eta - get_eta(2) > 2) {
        selected_particles.push_back(std::polar(1.0, get_phi(i)));
    }
}

// If the selected_particles vector contains exactly 4 particles, calculate the 2-particle cumulant
if (selected_particles.size() == 4) {
    std::complex<double> Q2 = selected_particles[0] * selected_particles[1] * std::conj(selected_particles[2]) * std::conj(selected_particles[3]);
    double two_particle_cumulant = std::real(Q2);
}

// If the selected_particles vector contains exactly 4 particles, calculate the 4-particle cumulant
if (selected_particles.size() == 4) {
    std::complex<double> Q4 = selected_particles[0] * selected_particles[1] * selected_particles[2] * std::conj(selected_particles[3]) + selected_particles[0] * selected_particles[2] * selected_particles[3] * std::conj(selected_particles[1]) + selected_particles[0] * selected_particles[1] * selected_particles[3] * std::conj(selected_particles[2]) + selected_particles[1] * selected_particles[2] * selected_particles[3] * std::conj(selected_particles[0]);
    double four_particle_cumulant = std::real(Q4);
}

// If the selected_particles vector contains exactly 4 particles, calculate exp(phi1 + phi2 - phi3 - phi4)
if (selected_particles.size() == 4) {
    std::complex<double> exp_sum = selected_particles[0] * selected_particles[1] * std::conj(selected_particles[2]) * std::conj(selected_particles[3]);
    double exp_phi_sum = std::arg(exp_sum);
}
