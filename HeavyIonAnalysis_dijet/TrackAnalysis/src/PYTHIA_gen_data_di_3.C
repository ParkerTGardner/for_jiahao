            ...
            for(int kjet=0; kjet < jetPt->size(); kjet++){
                ...
                for(int jjet=ijet+1; (jjet< jetPt->size()); jjet++){
                    ...
                    for(long int T_trk = 0; T_trk < NNtrk2; T_trk++ ){
                        ...
                        hEtaPhiT->Fill(T_jet_dau_eta, T_jet_dau_phi, 1.0* jet_HLT_weight2/Ttrk_weight);
                        hEtaT -> Fill(T_jet_dau_eta,1.0* jet_HLT_weight2/Ttrk_weight);
                        hPhiT -> Fill(T_jet_dau_phi,1.0* jet_HLT_weight2/Ttrk_weight);
                        hJtT  -> Fill(T_jet_dau_pt, 1.0* jet_HLT_weight2/Ttrk_weight);

                        hEtaPhiTT->Fill(TT_jet_dau_eta, TT_jet_dau_phi, 1.0* jet_HLT_weight2/Ttrk_weight);
                        hEtaTT -> Fill(TT_jet_dau_eta,1.0* jet_HLT_weight2/Ttrk_weight);
                        hPhiTT -> Fill(TT_jet_dau_phi,1.0* jet_HLT_weight2/Ttrk_weight);
                        hJtTT  -> Fill(TT_jet_dau_pt, 1.0* jet_HLT_weight2/Ttrk_weight);
                        ...
                        for(int i = 0; i < trackbin; i++){
                            for(int j = 0; j < ptbin; j++){
                                if(tkBool[i] + A_ptBool[A_trk][j] == 2){
                                    ...
                                    if ((Ntrig[i][j])==0) continue;
                                    double nUnc_weight = (hReco2D[thisEffTable]->GetBinContent(hReco2D[thisEffTable]->FindBin( (*dau_pt)[ijet][A_trk] , (*dau_eta)[ijet][A_trk] )));
                                    hEPDrawA[i][j][k_PU]->Fill(jet_dau_eta, jet_dau_phi, 1.0* jet_HLT_weight/(Atrk_weight*Ntrig[i][j]));
                                }
                            }
                        }
