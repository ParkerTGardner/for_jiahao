...
// cout << "TEST 1" << endl;
    ...
    //what is the def of genDau_pt, genJetPt
            ...
            for(int kjet=0; kjet < jetPt->size(); kjet++){
                ...
                for(int jjet= 0; (jjet< jetPt->size()); jjet++){
                    ...
                    hBinDist_corrected           ->Fill(n_G_ChargeMult_count_corrected1, 1.0*jet_HLT_weight*jet_HLT_weight2);
                    hBinDist_uncorrected        ->Fill(n_G_ChargeMult_count1, 1.0*jet_HLT_weight*jet_HLT_weight2);
