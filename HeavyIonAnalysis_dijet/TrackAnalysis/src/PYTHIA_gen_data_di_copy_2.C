...
// cout << "TEST 1" << endl;
    ...
    //what is the def of genDau_pt, genJetPt
            ...
            for(int kjet=0; kjet < jetPt->size(); kjet++){
                ...
                TLorentzVector JetA_4 (JetA, sqrt(JetA.Mag2()+pow((*jetMass)[ijet],2)));
                ...
                hBinDist_uncorrected        ->Fill(n_G_ChargeMult_count1, 1.0*jet_HLT_weight);
                
                
                for(int jjet=ijet+1; (jjet< jetPt->size()); jjet++){


                    TVector3 JetB;
                    JetB.SetPtEtaPhi((*jetPt)[jjet],(*jetEta)[jjet],(*jetPhi)[jjet]);
                    TLorentzVector JetB_4 (JetB, sqrt(JetB.Mag2()+pow((*jetMass)[jjet],2)));
                    ...
                    for(int G_trk2=0; G_trk2 < NNtrk2; G_trk2++ ){
                        ...
                        n_G_ChargeMult_count_corrected2  += (1.0/nUnc_weight);
                    }

                    profile_AB->Fill(n_G_ChargeMult_count1,n_G_ChargeMult_count2,1.0*jet_HLT_weight*jet_HLT_weight2);
                    profile_AB_corrected->Fill(n_G_ChargeMult_count_corrected1,n_G_ChargeMult_count_corrected2,1.0*jet_HLT_weight*jet_HLT_weight2);
                    ...
                    hMult_AB -> Fill(n_G_ChargeMult_count1, n_G_ChargeMult_count2, 1.0*jet_HLT_weight*1.0*jet_HLT_weight2);
                    ...
                    hBinDist_gen_single            ->Fill(n_G_ChargeMult_count1,1.0*jet_HLT_weight*1.0*jet_HLT_weight2);
                    ...
                    bool if_diagonal = false;

                    for(int i = 0; i < trackbin; i++){
                    //if((*chargedMultiplicity)[indicesR[kjet]] >= trackbinbounds[i] && (*chargedMultiplicity)[indicesR[kjet]] < trackbinboundsUpper[i]){
                        if(n_G_ChargeMult_count1 >= trackbinbounds[i] && n_G_ChargeMult_count1 < trackbinboundsUpper[i] && n_G_ChargeMult_count2 >= trackbinbounds[i] && n_G_ChargeMult_count2 < trackbinboundsUpper[i]){
                            tkBool[i] = 1;
                            hJet_Pass           ->Fill(i);
                            hBinDist_gen[i]         ->Fill(n_G_ChargeMult_count, 1.0*jet_HLT_weight*jet_HLT_weight2);
                            // hMult_AB_A[i]  -> Fill(n_G_ChargeMult_count1, 1.0*jet_HLT_weight);
                            // hMult_AB_AB[i] -> Fill (n_G_ChargeMult_count1, n_G_ChargeMult_count2, 1.0*1.0*jet_HLT_weight*1.0*jet_HLT_weight2); 
                            if_diagonal = true;
                        }
                    }
                    
                    if (!if_diagonal) continue;
                    ...
                    for(long int T_trk = 0; T_trk < NNtrk2; T_trk++ ){
                        ...
                        if((*dau_chg)[jjet][T_trk] == 0) continue;
                        if(fabs((*dau_eta)[jjet][T_trk]) > 2.4) continue;
                        if(fabs((*dau_pt)[jjet][T_trk])  < 0.3)      continue;
                        ...
                        hEtaPhiT->Fill(T_jet_dau_eta, T_jet_dau_phi, 1.0* jet_HLT_weight*jet_HLT_weight2/Ttrk_weight);
                        hEtaT -> Fill(T_jet_dau_eta,1.0* jet_HLT_weight*jet_HLT_weight2/Ttrk_weight);
                        hPhiT -> Fill(T_jet_dau_phi,1.0* jet_HLT_weight*jet_HLT_weight2/Ttrk_weight);
                        hJtT  -> Fill(T_jet_dau_pt, 1.0* jet_HLT_weight*jet_HLT_weight2/Ttrk_weight);

                        hEtaPhiTT->Fill(TT_jet_dau_eta, TT_jet_dau_phi, 1.0* jet_HLT_weight*jet_HLT_weight2/Ttrk_weight);
                        hEtaTT -> Fill(TT_jet_dau_eta,1.0* jet_HLT_weight*jet_HLT_weight2/Ttrk_weight);
                        hPhiTT -> Fill(TT_jet_dau_phi,1.0* jet_HLT_weight*jet_HLT_weight2/Ttrk_weight);
                        hJtTT  -> Fill(TT_jet_dau_pt, 1.0* jet_HLT_weight*jet_HLT_weight2/Ttrk_weight);
                        ...
                        for(int i = 0; i < trackbin; i++){
                            for(int j = 0; j < ptbin; j++){
                                if(tkBool[i] + T_ptBool[T_trk][j] == 2){
                                    ...
                                    hEPDrawT[i][j][k_PU]->Fill(T_jet_dau_eta, T_jet_dau_phi , 1.0* jet_HLT_weight*jet_HLT_weight2/(Ttrk_weight*Ntrig[i][j]) );
                                }
                            }
                        }
                    ...
                    for(int  A_trk=0; A_trk < NNtrk1; A_trk++ ){
                        ...
                        hEtaPhiA->Fill(jet_dau_eta, jet_dau_phi, 1.0* jet_HLT_weight*jet_HLT_weight2/Atrk_weight);
                        hEtaA -> Fill(jet_dau_eta,1.0* jet_HLT_weight*jet_HLT_weight2/Atrk_weight);
                        hPhiA -> Fill(jet_dau_phi,1.0* jet_HLT_weight*jet_HLT_weight2/Atrk_weight);
                        hJtA  -> Fill(jet_dau_pt, 1.0* jet_HLT_weight*jet_HLT_weight2/Atrk_weight);
                        ...
                        for(int i = 0; i < trackbin; i++){
                            for(int j = 0; j < ptbin; j++){
                                if(tkBool[i] + A_ptBool[A_trk][j] == 2){
                                    ...
                                    if ((Ntrig[i][j])==0) continue;
                                    double nUnc_weight = (hReco2D[thisEffTable]->GetBinContent(hReco2D[thisEffTable]->FindBin( (*dau_pt)[ijet][A_trk] , (*dau_eta)[ijet][A_trk] )));
                                    hEPDrawA[i][j][k_PU]->Fill(jet_dau_eta, jet_dau_phi, 1.0* jet_HLT_weight*jet_HLT_weight2/(Atrk_weight*Ntrig[i][j]));
                                }
                            }
                        }
                            ...
                            for(        int i = 0; i < trackbin; i++){
                                for(    int j = 0; j < ptbin;    j++){ 
                                    ...
                                    if(tkBool[i] + A_ptBool[A_trk][j] + T_ptBool[T_trk][j] == 3){
                                            ...
                                            if ((Ntrig[i][j])==0) continue;
                                            int k_PU=0;
                                            hSignalShifted[i][j][k_PU]->Fill(deltaEta, deltaPhi,                 1.0* jet_HLT_weight*jet_HLT_weight2/(Ttrk_weight*Atrk_weight*Ntrig[i][j]));
                                            hSignalShifted[i][j][k_PU]->Fill(-deltaEta, deltaPhi,                1.0* jet_HLT_weight*jet_HLT_weight2/(Ttrk_weight*Atrk_weight*Ntrig[i][j]));
                                            hSignalShifted[i][j][k_PU]->Fill(deltaEta, -deltaPhi,                1.0* jet_HLT_weight*jet_HLT_weight2/(Ttrk_weight*Atrk_weight*Ntrig[i][j]));
                                            hSignalShifted[i][j][k_PU]->Fill(-deltaEta, -deltaPhi,               1.0* jet_HLT_weight*jet_HLT_weight2/(Ttrk_weight*Atrk_weight*Ntrig[i][j]));
                                            hSignalShifted[i][j][k_PU]->Fill( deltaEta,2*TMath::Pi() - deltaPhi, 1.0* jet_HLT_weight*jet_HLT_weight2/(Ttrk_weight*Atrk_weight*Ntrig[i][j]));
                                            hSignalShifted[i][j][k_PU]->Fill(-deltaEta,2*TMath::Pi() - deltaPhi, 1.0* jet_HLT_weight*jet_HLT_weight2/(Ttrk_weight*Atrk_weight*Ntrig[i][j]));
