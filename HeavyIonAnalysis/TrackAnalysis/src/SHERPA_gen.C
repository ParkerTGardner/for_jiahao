...
void MyClass::Loop(int job, std::string fList){
    ...
    TH2D* hPairsP        = new TH2D("hPairsP","hPairsP",  trackbin, bin0,trackbin, ptbin, bin0, ptbin);
    TH2D* hPairsM        = new TH2D("hPairsM","hPairsM",  trackbin, bin0,trackbin, ptbin, bin0, ptbin);
    TH2D* hPairsO        = new TH2D("hPairsO","hPairsO",  trackbin, bin0,trackbin, ptbin, bin0, ptbin);

    TH2D* hNtrig        = new TH2D("hNtrig","hNtrig",  trackbin, bin0,trackbin, ptbin, bin0, ptbin);
    TH2D* hNtrigP        = new TH2D("hNtrigP","hNtrigP",  trackbin, bin0,trackbin, ptbin, bin0, ptbin);
    TH2D* hNtrigM        = new TH2D("hNtrigM","hNtrigM",  trackbin, bin0,trackbin, ptbin, bin0, ptbin);
...
std::cout<< "made 4" << endl;
                    ...
                    TFile* fS_tempA = new TFile(Form("august_sherpa/root_out_2/job_%s.root",subList.c_str()), "recreate");
                    for(int wtrk =1; wtrk <trackbin+1; wtrk++){
